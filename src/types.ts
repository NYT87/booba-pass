...
export function computeDurationMin(
  ...
  startTimeZone?: string, endTimeZone?: string,
): number {
...
export function flightDurationMin(f: Flight): number {
  if (f.actualDepartureDate && f.actualDepartureTime && f.actualArrivalDate && f.actualArrivalTime) {
    return computeDurationMin(
      f.actualDepartureDate, f.actualDepartureTime,
      f.actualArrivalDate, f.actualArrivalTime,
      f.departureTimeZone, f.arrivalTimeZone
    );
  }
  return computeDurationMin(
    f.scheduledDepartureDate, f.scheduledDepartureTime,
    f.scheduledArrivalDate, f.scheduledArrivalTime,
    f.departureTimeZone, f.arrivalTimeZone
  );
}
