  ...
  return (
    <div className="page animate-in">
      <header className="page-header">
        <button onClick={() => navigate('/flights')} className="btn-ghost"><ArrowLeft size={24} /></button>
        <h1>Flight Details</h1>
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={() => navigate(`/flights/${flight.id}/edit`)} className="btn-ghost"><Edit2 size={20} /></button>
          <button onClick={handleDelete} className="btn-ghost" style={{ color: 'var(--danger)' }}><Trash2 size={20} /></button>
        </div>
      </header>
      ...
      <div className="flight-detail-hero">
        <div className="detail-route">
          ...
          <div className="detail-plane"><Plane size={32} /></div>
      ...
      <div className="detail-info-grid">
        <div className="detail-info-card">
          <div className="detail-info-label"><Calendar size={12} style={{ marginBottom: -2, marginRight: 4 }} /> Date</div>
          <div className="detail-info-value">{flight.scheduledDepartureDate}</div>
        </div>
        <div className="detail-info-card">
          <div className="detail-info-label"><Clock size={12} style={{ marginBottom: -2, marginRight: 4 }} /> Duration</div>
          <div className="detail-info-value">{duration}</div>
        </div>
        <div className="detail-info-card">
          <div className="detail-info-label"><MapPin size={12} style={{ marginBottom: -2, marginRight: 4 }} /> Distance</div>
          <div className="detail-info-value">{Math.round(flight.distanceKm).toLocaleString()} km</div>
        </div>
        <div className="detail-info-card">
          <div className="detail-info-label"><CreditCard size={12} style={{ marginBottom: -2, marginRight: 4 }} /> Aircraft</div>
      ...
      {flight.trackUrl && (
        <a href={flight.trackUrl} target="_blank" rel="noopener noreferrer" className="btn-primary" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textDecoration: 'none' }}>
      ...
      <section className="form-section">
        <div className="form-section-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          Photos
          <label className="btn-ghost" style={{ fontSize: '0.75rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}>
            <input type="file" accept="image/*" multiple onChange={handlePhotoUpload} hidden />
            <Camera size={16} /> Add
          </label>
        </div>
        <div className="photo-gallery">
          {flight.photoDataUrls && flight.photoDataUrls.map((p, i) => (
          ...
          {(!flight.photoDataUrls || flight.photoDataUrls.length === 0) && (
            <div className="card" style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.85rem' }}>
              No photos yet.
            </div>
          )}
        </div>
      ...
      {flight.notes && (
        <section className="form-section">
          ...
          <div className="card" style={{ padding: 16, color: 'var(--text-secondary)', fontStyle: 'italic', lineHeight: 1.5 }}>
            "{flight.notes}"
          </div>
        </section>
      )}
