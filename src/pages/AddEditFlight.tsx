  ...
  return (
    <div className="page animate-in">
      <header className="page-header">
        <button onClick={() => navigate(id ? `/flights/${id}` : '/flights')} className="btn-ghost"><X size={24} /></button>
      ...
      <div className="form-section">
        ...
        <div className="form-row" style={{ alignItems: 'flex-start' }}>
          ...
          <button className="swap-btn" onClick={swapAirports}><ArrowLeftRight size={18} /></button>
      ...
      <div className="form-section">
        ...
        <div className="form-row">
          ...
          <div className="form-field">
            <label>Time</label>
            <input type="time" value={scheduledDepartureTime} onChange={e => setScheduledDepartureTime(e.target.value)} />
          </div>
        </div>
      ...
      <div className="form-section">
        ...
        <div className="form-field">
          ...
          <input type="url" value={trackUrl} onChange={e => setTrackUrl(e.target.value)} placeholder="FlightRadar24 / FlightAware link" />
        </div>
        <div className="form-field" style={{ marginTop: 12 }}>
          <label>Notes</label>
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Any memories from the flight?" />
        </div>
      </div>

      {(departure && arrival && scheduledDepartureDate && scheduledDepartureTime && scheduledArrivalDate && scheduledArrivalTime) && (
        <div className="form-section">
          <div className="card" style={{ padding: 16, textAlign: 'center', background: 'rgba(37, 175, 244, 0.1)', border: '1px solid rgba(37, 175, 244, 0.2)' }}>
            <div style={{ color: 'var(--accent)', fontSize: '0.8rem', marginBottom: 4 }}>Calculated Duration</div>
            <div style={{ fontSize: '1.2rem', fontWeight: 700 }}>
              {formatDuration(computeDurationMin(
        ...
        <div className="photo-upload-grid">
          {photos.map((p, i) => (
            <div key={i} className="photo-preview-item card">
              ...
              <button className="photo-remove-btn" onClick={() => removePhoto(i)}><Trash2 size={14} /></button>
            </div>
          ))}
